...
def revert_author(author):
    ...
    for code in names.keys():
        if names[code] == author:
            return code
    raise KeyError("Author is not in the list")
...
def author_filt_selector(conn, channel, format_lang, authors):
    ...
    WHERE author IN {1}
    ;
    """.format(channel, tuple([revert_author(x) for x in authors]))
